...
function assertRecognizingLayerMarkup(wxml, pageName) {
  ...
  assert.doesNotMatch(wxml, /class="recognizing-mascot"/, `${pageName} should not use the simplified mascot-only header`);
...
test('recognizing overlay styles preserve orbit, step list, and hint fidelity', () => {
  for (const fileName of ['index.wxss', 'review.wxss']) {
    ...
    assert.match(cssBlock(wxss, '.recognizing-orbit'), /conic-gradient/, `${fileName} should draw the orbit progress ring`);
